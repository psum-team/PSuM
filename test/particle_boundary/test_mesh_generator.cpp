...
#include "../../src/particle_boundary/mesh_generator.hpp"
...
inline auto translate(const std::vector<geometry::triangle_data>& triangles_, double dx, double dy, double dz) {
...
void test_mesh_generator() {
    std::cout << "==========================================" << std::endl;
    std::cout << "check: mesh generator demonstration" << std::endl;

    sycl::queue q;
    std::cout << "Running on " << q.get_device().get_info<sycl::info::device::name>() << std::endl;

    Tic("build_mesh")
    auto mesh0 = mesh_generator::axis_aligned_cube(0, 0, 0, 1, 1, 1);
    auto mesh1 = mesh_generator::extrude_xy_line_to_width({{0,0}, {1,0}, {1,1}, {2,1}}, 1.5);
    auto mesh2 = mesh_generator::revolve_zr_curve({{0,0.5}, {0.5,0.5}, {1,1}, {1.5,0.5}, {2,0.5}}, 128);

    triangle_mesh_trigger<int> trigger(q);
    trigger.set(
        {
            {translate(mesh0, 0, 0, 0), 0},
            {translate(mesh1, 1.5, 0, 0), 1},
            {translate(mesh2, 5.0, 0, 0), 2},
        }, 100, 100, 100);
    ...
    std::cout << "✅ Mesh generator test completed!" << std::endl;
    PrintTimer
