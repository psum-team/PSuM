#ifndef PSUM_FIELD_SOLVER_BOUNDARY_CREATOR_ROBIN_HPP
#define PSUM_FIELD_SOLVER_BOUNDARY_CREATOR_ROBIN_HPP
...
namespace boundary_creator {
    ...
    mixed_boundary_1d Robin_point(const field::grid1D &g, boundary_direction_1d dir, 
        const std::function<std::pair<double, double>(double)>& coef_value_and_grad)
    {
        ...
        ans.set_coeff_func([coef_value_and_grad](const std::array<double, 1>& pos) {
            return coef_value_and_grad(pos[0]);
        });
        return ans;
    }

    mixed_boundary_1d Robin_point(const field::grid1D &g, boundary_direction_1d dir, 
        std::pair<double, double> coef_value_and_grad)
    {
        return Robin_point(g, dir, 
            [coef_value_and_grad](double)
            { return coef_value_and_grad; });
    }

    mixed_boundary_2d Robin_Line(const field::grid2D &g, boundary_direction_2d dir, 
        const std::function<std::pair<double, double>(double, double)>& coef_value_and_grad,
        std::pair<double, double> min_max = {nan(""), nan("")})
    {
        ...
        ans.set_direction(dir);
        ans.set_related_elements(related_elements);
        ans.set_coeff_func([coef_value_and_grad](const std::array<double, 2>& pos) {
            return coef_value_and_grad(pos[0], pos[1]);
        });
        return ans;
    }
    ...
    mixed_boundary_2d Robin_Line(const field::grid2D &g, boundary_direction_2d dir, 
        ...
        std::pair<double, double> min_max = {nan(""), nan("")})
    {
        return Robin_Line(g, dir, 
            [coef_value_and_grad](double, double)
            { return coef_value_and_grad; },
             min_max);
    }

    mixed_boundary_3d Robin_plane(const field::grid3D &g, boundary_direction_3d dir, 
        const std::function<std::pair<double, double>(double, double, double)>& coef_value_and_grad,
        std::pair<double, double> min_max = {nan(""), nan("")})
    {
        ...
        ans.set_direction(dir);
        ans.set_related_elements(related_elements);
        ans.set_coeff_func([coef_value_and_grad](const std::array<double, 3>& pos) {
            return coef_value_and_grad(pos[0], pos[1], pos[2]);
        });
        return ans;
    }
    ...
    mixed_boundary_3d Robin_plane(const field::grid3D &g, boundary_direction_3d dir, 
        ...
        std::pair<double, double> min_max = {nan(""), nan("")})
    {
        return Robin_plane(g, dir, 
            [coef_value_and_grad](double, double, double)
            { return coef_value_and_grad; },
             min_max);
    }
