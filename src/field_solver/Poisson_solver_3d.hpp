#ifndef PSUM_FIELD_SOLVER_POISSON_SOLVER_3D_HPP
#define PSUM_FIELD_SOLVER_POISSON_SOLVER_3D_HPP
...
class Poisson_solver_3d{
    using Grid = psum::field::simple_grid<3>;
...
public:
    ...
    Poisson_solver_3d() : backend_name_("native"), grid_({0.0, 0.0, 0.0}, {1.0, 1.0, 1.0}, {1, 1, 1}) {};
    inline void init(const Grid &grid, psolverDomainKind domainType, const mixed_boundary_3d_list &boundaries, const fixed_boundary_3d_list &fbl, double _epsilon = 8.854e-12) {
        _init_impl("native", grid, domainType, boundaries, fbl, [_epsilon](double, double, double) { return _epsilon; });
    }
    inline void init(const Grid &grid, psolverDomainKind domainType, const mixed_boundary_3d_list &boundaries, const fixed_boundary_3d_list &fbl, const std::function<double(double, double, double)>& _epsilon) {
        _init_impl("native", grid, domainType, boundaries, fbl, _epsilon);
    }
    inline void init(const Grid& grid, psolverDomainKind domainType, const fixed_boundary_3d_list& fbl, double _epsilon = 8.854e-12){
        _init_impl("native", grid, domainType, {}, fbl, [_epsilon](double, double, double) { return _epsilon; });
    }
    inline void init(const Grid& grid, psolverDomainKind domainType, const fixed_boundary_3d_list& fbl, const std::function<double(double, double, double)>& _epsilon){
        _init_impl("native", grid, domainType, {}, fbl, _epsilon);
    }
    inline void init(const std::string& backend_name, const Grid &grid, psolverDomainKind domainType, const mixed_boundary_3d_list &boundaries, const fixed_boundary_3d_list &fbl, double _epsilon = 8.854e-12) {
        _init_impl(backend_name, grid, domainType, boundaries, fbl, [_epsilon](double, double, double) { return _epsilon; });
    }
    inline void init(const std::string& backend_name, const Grid &grid, psolverDomainKind domainType, const mixed_boundary_3d_list &boundaries, const fixed_boundary_3d_list &fbl, const std::function<double(double, double, double)>& _epsilon) {
        _init_impl(backend_name, grid, domainType, boundaries, fbl, _epsilon);
    }
    inline void init(const std::string& backend_name, const Grid& grid, psolverDomainKind domainType, const fixed_boundary_3d_list& fbl, double _epsilon = 8.854e-12){
        _init_impl(backend_name, grid, domainType, {}, fbl, [_epsilon](double, double, double) { return _epsilon; });
    }
    inline void init(const std::string& backend_name, const Grid& grid, psolverDomainKind domainType, const fixed_boundary_3d_list& fbl, const std::function<double(double, double, double)>& _epsilon){
    ...
    inline void solve(Eigen::Ref<Eigen::VectorXd> phi, const Eigen::Ref<const Eigen::VectorXd>& source) {
        ...
        if (source.size() != b_.size())
            std::cerr << "Poisson_solver_3d solve: source and grid differ in size" << std::endl;
        b_ = source;
        if (phi.size() != b_.size())
            phi.resize(A_.cols());
    ...
    inline void make_righthand_item(const Eigen::VectorXd& rho, Eigen::VectorXd& b) {
        fbl_.evaluate(replace_elements_.second);
        mbl_.evaluate(mixed_boundary_values_);
...
protected:
    ...
    Eigen::BiCGSTAB<Eigen::SparseMatrix<double, Eigen::RowMajor>, Eigen::IncompleteLUT<double>> solver_;
    Eigen::VectorXd b_;
    fixed_boundary_3d_list fbl_;
    mixed_boundary_3d_list mbl_;
    ...
    inline void _init_impl(const std::string& backend_name, const Grid& grid, psolverDomainKind domainType, const mixed_boundary_3d_list& mbl, const fixed_boundary_3d_list& fbl, const std::function<double(double, double, double)>& epsilon){
        ...
        int K = grid_.numCells<2>();
        double dx_r = grid_.del_r<0>();
        double dy_r = grid_.del_r<1>();
        double dz_r = grid_.del_r<2>();
        auto cyclic_index = [I, J, K](std::array<int, 3> idx) {
            std::array<int, 3> cyc_idx = idx;
            cyc_idx[0] = (cyc_idx[0] % (I + 1) + I + 1) % (I + 1);
            cyc_idx[1] = (cyc_idx[1] % (J + 1) + J + 1) % (J + 1);
            cyc_idx[2] = (cyc_idx[2] % (K + 1) + K + 1) % (K + 1);
            return Grid::node_index(cyc_idx);
        };
        for (int i = 0; i <= I; i++) {
            for (int j = 0; j <= J; j++) {
                for (int k = 0; k <= K; k++) {
                    size_t idx = grid_.n2i({i, j, k});
                    local_equation final_eq;
                    local_equation local_value({{idx, 1.0}});

                    size_t idx_N = grid_.n2i(cyclic_index({i, j + 1, k}));
                    size_t idx_S = grid_.n2i(cyclic_index({i, j - 1, k}));
                    size_t idx_E = grid_.n2i(cyclic_index({i + 1, j, k}));
                    size_t idx_W = grid_.n2i(cyclic_index({i - 1, j, k}));
                    size_t idx_U = grid_.n2i(cyclic_index({i, j, k + 1}));
                    size_t idx_D = grid_.n2i(cyclic_index({i, j, k - 1}));
                    ...
                    if(fbl_.element_exists(idx)) {
                        ...
                        for (auto node_idx: element_idx_to_mb_node_idx[idx]) {
                            boundary_direction_3d dir = mb_nodes[node_idx].direction;
                            switch (dir) {
                            case boundary_direction_3d::Ypos:
                                idx_N = ghost_mng.new_ghost_element();
                                break;
                            case boundary_direction_3d::Yneg:
                                idx_S = ghost_mng.new_ghost_element();
                                break;
                            case boundary_direction_3d::Xpos:
                                idx_E = ghost_mng.new_ghost_element();
                                break;
                            case boundary_direction_3d::Xneg:
                                idx_W = ghost_mng.new_ghost_element();
                                break;
                            case boundary_direction_3d::Zpos:
                                idx_U = ghost_mng.new_ghost_element();
                                break;
                            case boundary_direction_3d::Zneg:
                                idx_D = ghost_mng.new_ghost_element();
                                break;
                            default:
                                break;
                            }
                        ...
                        for (auto node_idx: element_idx_to_mb_node_idx[idx]) {
                            ...
                            case boundary_direction_3d::Xneg:
                                ...
                                break;
                            case boundary_direction_3d::Zpos:
                                final_eq += grad_Z * g_coef + local_value * v_coef;
                                break;
                            case boundary_direction_3d::Zneg:
                                final_eq += grad_Z * -g_coef + local_value * v_coef;
                                break;
                        ...
                        local_equation grad_U;
                        local_equation grad_D;
                        ...
                        grad_U = local_equation({{idx, dz_r * -1}, {idx_U, dz_r}});
                        grad_D = local_equation({{idx_D, dz_r * -1}, {idx, dz_r}});

                        // compute epsilon
                        double eps_N, eps_S, eps_E, eps_W, eps_U, eps_D;
                        Eigen::RowVector3d pos_N = (grid_.nodePosition({i, j, k}) + grid_.nodePosition({i, j + 1, k})) / 2;
                        Eigen::RowVector3d pos_S = (grid_.nodePosition({i, j, k}) + grid_.nodePosition({i, j - 1, k})) / 2;
                        Eigen::RowVector3d pos_E = (grid_.nodePosition({i, j, k}) + grid_.nodePosition({i + 1, j, k})) / 2;
                        Eigen::RowVector3d pos_W = (grid_.nodePosition({i, j, k}) + grid_.nodePosition({i - 1, j, k})) / 2;
                        Eigen::RowVector3d pos_U = (grid_.nodePosition({i, j, k}) + grid_.nodePosition({i, j, k + 1})) / 2;
                        Eigen::RowVector3d pos_D = (grid_.nodePosition({i, j, k}) + grid_.nodePosition({i, j, k - 1})) / 2;
                        eps_N = epsilon(pos_N.x(), pos_N.y(), pos_N.z());
                        eps_S = epsilon(pos_S.x(), pos_S.y(), pos_S.z());
                        eps_E = epsilon(pos_E.x(), pos_E.y(), pos_E.z());
                        eps_W = epsilon(pos_W.x(), pos_W.y(), pos_W.z());
                        eps_U = epsilon(pos_U.x(), pos_U.y(), pos_U.z());
                        eps_D = epsilon(pos_D.x(), pos_D.y(), pos_D.z());

                        double surface_W, surface_E, surface_S, surface_N, surface_D, surface_U;
                        double volume;
                        surface_N = surface_S = grid_.del<0>() * grid_.del<2>();
                        surface_E = surface_W = grid_.del<1>() * grid_.del<2>();
                        surface_U = surface_D = grid_.del<0>() * grid_.del<1>();
                        volume = grid_.del<0>() * grid_.del<1>() * grid_.del<2>();

                        // Poisson equation: s = -div(grad(phi)) ~ sum(flux on cell surface) / -vol
                        final_eq = (grad_E * surface_E * eps_E - grad_W * surface_W * eps_W + 
                                    grad_N * surface_N * eps_N - grad_S * surface_S * eps_S + 
                                    grad_U * surface_U * eps_U - grad_D * surface_D * eps_D) * (-1.0 / volume);
                    }
        ...
        if (backend_name_ == "native") {
            solver_.preconditioner().setDroptol(1e-3);
            solver_.preconditioner().setFillfactor(10);
            solver_.setTolerance(1e-11);
