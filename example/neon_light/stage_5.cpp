...
int main() {
    ...
    solver.init(
        ...
        Poisson_solver_2d::Cartesian,
        {
            ...
            Dirichlet_line(grid, boundary_direction_2d::N, 
                {grid.lowerBound<0>() + 0.3 * grid.span<0>(), grid.upperBound<0>() - 0.3 * grid.span<0>()}) = 2.5,
            ...
            Dirichlet_func(grid, [&](double x, double y) {
                ...
                return mask_val > 0.01 && mask_val < 0.99;
            }) = [&](const auto& pos) {
                return voltage(t, pos[0], pos[1]) * 0.5;
            }
        }
    );
    ...
    cout << "Neon Light stage 5: wrote output/phi_step_*.plt" << endl;
    return 0;
}
